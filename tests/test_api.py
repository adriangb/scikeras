...
def build_fn_clf(hidden_dim, meta: Dict[str, Any], compile_kwargs: Dict[str, Any],) -> Model:
...
def build_fn_reg(hidden_dim, meta: Dict[str, Any], compile_kwargs: Dict[str, Any],) -> Model:
...
class InheritClassBuildFnClf(KerasClassifier):
    def _keras_build_fn(self, hidden_dim, meta: Dict[str, Any], compile_kwargs: Dict[str, Any],) -> Model:
        return build_fn_clf(hidden_dim=hidden_dim, meta=meta, compile_kwargs=compile_kwargs,)


class InheritClassBuildFnReg(KerasRegressor):
    def _keras_build_fn(self, hidden_dim, meta: Dict[str, Any], compile_kwargs: Dict[str, Any],) -> Model:
        return build_fn_reg(hidden_dim=hidden_dim, meta=meta, compile_kwargs=compile_kwargs,)
...
def build_fn_regs(hidden_layer_sizes, meta: Dict[str, Any], compile_kwargs: Dict[str, Any],) -> Model:
...
def build_fn_clss(hidden_layer_sizes, meta: Dict[str, Any], compile_kwargs: Dict[str, Any],) -> Model:
...
def build_fn_clscs(hidden_layer_sizes, meta: Dict[str, Any], compile_kwargs: Dict[str, Any],) -> Model:
...
def build_fn_clscf(hidden_layer_sizes, meta: Dict[str, Any], compile_kwargs: Dict[str, Any],) -> Model:
...
CONFIG = {
    "MLPRegressor": (load_boston, KerasRegressor, dynamic_regressor, (BaggingRegressor, AdaBoostRegressor),),
    "MLPClassifier": (load_iris, KerasClassifier, dynamic_classifier, (BaggingClassifier, AdaBoostClassifier),),
    "CNNClassifier": (load_digits8x8, KerasClassifier, build_fn_clscs, (BaggingClassifier, AdaBoostClassifier),),
    "CNNClassifierF": (load_digits8x8, KerasClassifier, build_fn_clscf, (BaggingClassifier, AdaBoostClassifier),),
}
...
class TestAdvancedAPIFuncs:
    ...
    def test_searchcv_init_params(self, config):
        ...
        loader, model, build_fn, _ = CONFIG[config]
        estimator = model(build_fn, epochs=1, validation_split=0.1, model__hidden_layer_sizes=[],)
        basic_checks(
            GridSearchCV(estimator, {"model__hidden_layer_sizes": [[], [5]]}), loader,
        )
        basic_checks(
            RandomizedSearchCV(estimator, {"epochs": [1, 2, 3], "optimizer": ["rmsprop", "sgd"]}, n_iter=2,), loader,
        )
    ...
    def test_calibratedclassifiercv(self, config):
        ...
        base_estimator = KerasClassifier(build_fn, epochs=1, model__hidden_layer_sizes=[])
...
def test_warm_start():
    ...
    estimator = KerasRegressor(
        model=dynamic_regressor, loss=KerasRegressor.r_squared, model__hidden_layer_sizes=(100,),
    )
    estimator.fit(X, y)
...
class TestPartialFit:
    ...
    def test_partial_fit_single_epoch(self):
        ...
        estimator = KerasRegressor(
            model=dynamic_regressor, loss=KerasRegressor.r_squared, model__hidden_layer_sizes=[100,], epochs=epochs,
        )
    ...
    def test_current_epoch_property(self, warm_start, epochs_prefix):
        ...
        estimator = KerasRegressor(
            model=dynamic_regressor, loss=KerasRegressor.r_squared, model__hidden_layer_sizes=[], warm_start=warm_start,
        )
    ...
    def test_pf_pickle_pf(self, config):
        ...
        rel_errors = [np.linalg.norm(w1 - w2) / np.linalg.norm((w1 + w2) / 2) for w1, w2 in zip(weights1, weights2)]
    ...
    def test_partial_fit_classes_param(self):
        ...
        """
        clf = KerasClassifier(
            model=dynamic_classifier, loss="sparse_categorical_crossentropy", model__hidden_layer_sizes=[100,],
        )
...
def force_compile_shorthand(hidden_layer_sizes, meta, compile_kwargs, params):
    model = dynamic_regressor(hidden_layer_sizes=hidden_layer_sizes, meta=meta, compile_kwargs=compile_kwargs)
    model.compile(
        optimizer=compile_kwargs["optimizer"], loss=compile_kwargs["loss"], metrics=params["metrics"],
    )
    return model
...
def test_compile_model_from_params():
    ...
    loss_obj = other_loss()
    estimator = DefaultLossNotNone(model=build_fn, loss=losses_module.CategoricalHinge(), my_loss=loss_obj,)
    with pytest.raises(ValueError, match=" but model compiled with "):
        estimator.fit(X, y)
