...
SavedKerasModel = namedtuple("SavedKerasModel", "cls model training_config weights",)
...
def _check_array_sizes(y_true, y_pred, sample_weight):
    ...
    if sample_weight is not None:
        ...
        if sample_weight.ndim != 1:
            raise ValueError("`sample_weight` must be an array of shape (n_samples,)")
...
def _accuracy_score(y_true, y_pred, sample_weight=None, **kwargs):
    ...
    """
    y_true, y_pred, sample_weight = _check_array_sizes(y_true, y_pred, sample_weight)
...
def _clone_prebuilt_model(build_fn):
    ...
    model.compile(**saving_utils.compile_args_from_training_config(training_config))
...
class BaseWrapper:
    ...
    def _check_params(self, params):
        ...
        for param_name in params:
            for fn in self._legal_params_fns:
                ...
                raise ValueError("{} is not a legal parameter".format(param_name))
    ...
    def _build_keras_model(self, X, y, sample_weight, **kwargs):
        ...
        X_y_args = self._filter_params(self.__call__, params_to_check={"X": X, "y": y})
...
class KerasClassifier(BaseWrapper):
    ...
    def _check_output_model_compatibility(self, y):
        ...
        for i, (loss, y_) in enumerate(zip(losses, y)):
            if is_categorical_crossentropy(loss) and (y_.ndim == 1 or y_.shape[1] == 1):
...
class KerasRegressor(BaseWrapper):
    ...
    def score(self, X, y, sample_weight=None, **kwargs):
        ...
        if self.model.loss not in ("mean_squared_error", self.root_mean_squared_error,):
            warnings.warn(
