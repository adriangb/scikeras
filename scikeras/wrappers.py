...
class BaseWrapper(BaseEstimator):
    ...
    def _check_model_outputs(self):
        ...
        n_outputs_expected = getattr(self, "n_outputs_expected_", None)
        # n_outputs_expected_ is generated by data transformers;
        # we recognize the attribute but do not force it to be
        # generated
        if n_outputs_expected:
            n_ouputs = len(self.model_.outputs)
            if n_outputs_expected != n_ouputs:
                raise ValueError(
                    "Expected the Keras model to have"
                    f" {n_outputs_expected} outputs but got"
                    f" {n_ouputs}"
                )
